...
# class AgentReturnPayload(BaseModel):
#     issues: str
class SimpleCodeReviewAgent:
    ...
    def _analysis_agent(self,state:CodeReviewState) -> Dict:
        ...
        """
        # response=self.llm.with_structured_output(AgentReturnPayload).invoke(prompt)
    ...
    def _find_issues(self,state: CodeReviewState)-> Dict:
        ...
        List 3-5 specific issues. Format each as "-issues".
        """
        response=self.llm.invoke(prompt)
